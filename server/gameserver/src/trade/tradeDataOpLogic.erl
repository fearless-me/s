...
opTradeOrder({TradeClass, OrderID, SelfID, DestRoleID}) ->
    ...
    case Result of
        ...
        [R] ->
            ...
            NR2 = NR#?MNESIA_Trade_Silver{sellState = ?TradeST_Finish, relateRoleID = SelfID},
...
downTimeOutOrder() ->
    ...
    Sql1 = qlc:q([Trade || Trade <- mnesia:table(?MNESIA_Trade_Silver),
        Trade#?MNESIA_Trade_Silver.sellState =:= ?TradeST_Normal, NowTime > Trade#?MNESIA_Trade_Silver.downTime]),

    %% 查询元宝交易
    Sql2 = qlc:q([tradeDataQueryLogic:changeTableNameToSilver(Trade) || Trade <- mnesia:table(?MNESIA_Trade_Gold),
    ...
    %% 处理
    dealDownTimeOutOrder(?MNESIA_Trade_Silver, Sql1),
    dealDownTimeOutOrder(?MNESIA_Trade_Gold, Sql2),
...
dealDownTimeOutOrder(TableName, SqlBin) ->
    ...
    Fun = fun(#?MNESIA_Trade_Silver{orderID = OrderID} = Trade) ->
        %% 更新表
        case deleteRecord(TableName, OrderID) of
            true ->
                ...
                %% 设置为已完成交易
                NewTrade = Trade#?MNESIA_Trade_Silver{sellState = ?TradeST_Finish},
...
%%				IsEquip = ItemID >= ?Item_Differentiate,
                ...
                %% 退出物品给玩家
                MailID = mail:sendSystemMail(Trade#?MNESIA_Trade_Silver.roleID,
					stringCfg:getString(tradeMail_Title, []),
					stringCfg:getString(tradeMail_BackYouItem, []), [Attach], erlang:integer_to_list(Trade#?MNESIA_Trade_Silver.orderID)),
...
onDeleteRole(RoleID) ->
    ...
    %% 查询铜币交易
    Sql1 = qlc:q([Trade || Trade <- mnesia:table(?MNESIA_Trade_Silver),
        Trade#?MNESIA_Trade_Silver.sellState /= ?TradeST_Finish, RoleID =:= Trade#?MNESIA_Trade_Silver.roleID]),
    ...
    %% 处理
    onDeleteRole(?MNESIA_Trade_Silver, edb:selectRecord(Sql1)),
    onDeleteRole(?MNESIA_Trade_Gold, edb:selectRecord(Sql2)),
...
onDeleteRole(TableName, List) ->
    Fun =
        fun(#?MNESIA_Trade_Silver{orderID = OrderID} = Trade) ->
            %% 更新表
            case deleteRecord(TableName, OrderID) of
                true ->
                    ?DEBUG_OUT("dealDownTimeOutOrder:~p", [Trade]),
                    ...
                    %% 设置为已完成交易
                    NewTrade = Trade#?MNESIA_Trade_Silver{sellState = ?TradeST_Finish, relateRoleID = ?Trade_Role_ID},
    ...
    case List of
        ...
        _ ->
            Fun = fun(Table, Tuples) ->
                ...
                %% 转换成指定表名
                Record = misc:list_to_record(ChileList, ?MNESIA_Trade_Silver),

                Tuple = #trade{
                    orderID         = Record#?MNESIA_Trade_Silver.orderID,
                    itemUID         = Record#?MNESIA_Trade_Silver.itemUID,
                    roleID          = Record#?MNESIA_Trade_Silver.roleID,
                    sellType        = Record#?MNESIA_Trade_Silver.sellType,
                    putTime         = listToString(time:convertSec2DateTimeStr(Record#?MNESIA_Trade_Silver.putTime)),
                    downTime        = listToString(time:convertSec2DateTimeStr(Record#?MNESIA_Trade_Silver.downTime)),
                    silver          = Record#?MNESIA_Trade_Silver.silver,
                    gold            = Record#?MNESIA_Trade_Silver.gold,
                    destRoleID      = Record#?MNESIA_Trade_Silver.destRoleID,
                    sellState       = Record#?MNESIA_Trade_Silver.sellState,
                    relateRoleID    = Record#?MNESIA_Trade_Silver.relateRoleID,
                    itemID          = Record#?MNESIA_Trade_Silver.itemID,
                    quality         = Record#?MNESIA_Trade_Silver.quality
                },
...
inittradeInfo(#trade{} = TradeInfo, ItemType, ItemSubType, ItemLevel, ItemProfession) ->
    case TradeInfo#trade.itemID < ?Item_RuneIDStart of
        true ->
            Quality = TradeInfo#trade.quality,
            PileNumber = TradeInfo#trade.pileNumber;
        _ ->
	...
	MnesiaRec = #?MNESIA_Trade_Silver{
		...
		itemClass       = getItemClass(TradeInfo#trade.itemID), % 获取道具大类
		itemType        = ItemType,
...
isHaveNoneStaticRow() ->
    Sql1 = qlc:q([{Trade#?MNESIA_Trade_Silver.orderID, Trade#?MNESIA_Trade_Silver.itemUID, Trade#?MNESIA_Trade_Silver.itemID} ||
        Trade <- mnesia:table(?MNESIA_Trade_Silver), Trade#?MNESIA_Trade_Silver.itemType =:= 0]),

    %% 查询元宝交易
    Sql2 = qlc:q([{Trade#?MNESIA_Trade_Gold.orderID, Trade#?MNESIA_Trade_Gold.itemUID, Trade#?MNESIA_Trade_Gold.itemID} ||
    ...
    isHaveNoneStaticRow(?TradeType_Silver, Sql1),
    isHaveNoneStaticRow(?TradeType_Gold, Sql2),
...
%% 获取道具的分类
getItemClass(ItemID) ->
    tradeLoadStaticInfo:getItemClass(ItemID).
...
saveTradeItemStaticInfo(TradeClass, #tradeItemStaticRec{orderID = OrderID} = StaticRec) ->
	...
	case queryTradeOrder(TradeClass, OrderID) of
		[] ->
			...
			skip;
		[#?MNESIA_Trade_Silver{} = Ret] ->
			%% 找到，更新
			New = Ret#?MNESIA_Trade_Silver{
			...
			updateRecord(OrderID, ?MNESIA_Trade_Silver, New);
...
queryTradeOrder(TradeClass, OrderID) ->
    case TradeClass of
        ?TradeType_Silver ->
            %% 查询铜币交易表
            queryTradeItem(OrderID, ?MNESIA_Trade_Silver);
        ?TradeType_Gold ->
            %% 查询元宝交易表
...
queryTradeOrder(TradeClass, OrderID, DestRoleID) ->
    ...
    case R of
        ...
        [R2] ->
            ...
            IsValide = if
                           RS#?MNESIA_Trade_Silver.sellType =:= ?TradeType_Private ->
                               %% 还需要判断是不是指定的人
                               tradeDataQueryLogic:isValidOrder(RS#?MNESIA_Trade_Silver.sellState,
                                   RS#?MNESIA_Trade_Silver.downTime)
                                   andalso RS#?MNESIA_Trade_Silver.destRoleID =:= DestRoleID;
                           true ->
                               tradeDataQueryLogic:isValidOrder(RS#?MNESIA_Trade_Silver.sellState,
                                   RS#?MNESIA_Trade_Silver.downTime)
                       end,
