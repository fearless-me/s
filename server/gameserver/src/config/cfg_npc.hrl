...
-record(npcCfg, {
        ...
        %%: 32 深红熔渊下一层传送（配合enter_map字段）
        %%: 64 深红熔渊上一层传送（配合enter_map字段）
        %%: 128 战场跨服排行榜          256 角斗跨服排行榜
        %%: 512 结婚申请人和离婚申请（已废弃）
        %%: 1024 传送人（配合enter_map字段）
        %%: 2048 情缘任务接取npc      4096 离婚npc
        button_type,
        ...
        %%: npc对话框模型展示坐标偏移，格式为[X，Y，Z]（与show2面板通用）
        offset,
