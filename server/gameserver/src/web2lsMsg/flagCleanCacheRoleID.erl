%% @author zhengzhichun
%% @doc @todo Add description to flagCleanCacheRoleID.


-module(flagCleanCacheRoleID).
...
doMsg( CmdSerial, Data ) ->
	?DEBUG_OUT("Web2lsGMFlagCleanCacheRoleID:~p",[Data]),
	
	case parseArgs(Data) of
		{OpUser,RoleID} ->
			lsSendMsg:sendMsg2DBServer(flagCleanCacheRoleID, {CmdSerial,RoleID}),
			
			Msg = {CmdSerial,self(),
				   RoleID
				  },
			put({gmCmdLog,CmdSerial},#recWeb2LsCommandLog{
														  opCode = ?Web2lsGMFlagCleanCacheRoleID,
														  opUser = OpUser,
														  opSerial = CmdSerial,
														  arg1  = RoleID
														 }),
...
dbAck(CmdSerial,DealCode,_S,State) ->	
	?LOG_OUT("Web2lsGMFlagCleanCacheRoleID CmdSerial:~p  dbAck-Data:~p",[CmdSerial,DealCode]),	
...
return2Web( CmdSerial, DealCode ) ->
	...
	SendData = {obj,[
					 {"command",?Web2lsGMFlagCleanCacheRoleIDAck},
					 {"serial",CmdSerial},
					 {"code",DealCode}
					]},
