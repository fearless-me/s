...
getMailItemAll() ->
	...
	case ets:tab2list(Ets) of
		...
		List ->
			FAttach =
				fun(#recMailAttachMent{mailID = MailID} = Attach, {false, {Items, Coins}}) ->
					Ret =
						case libMail:isItemOrCoin(Attach) of
							true ->
								%% 货币
								case getMailCoin(MailID) of
									ok ->
										{Items, updateList(true, Coins, Attach#recMailAttachMent.mtype, Attach#recMailAttachMent.mvalue)};
									_ ->
										false
								end;
							undefined ->
								%% 错误的邮件
								?ERROR_OUT("player error mail:~p,~p", [playerState:getRoleID(), Attach]),
								false;
							{ItemUID, ItemID} ->
								%% 道具
								ItemNumber =
									case edb:dirtyReadRecord(rec_item, ItemUID) of
										[#rec_item{pileNum = PileNum}] -> PileNum;
										_ ->
											?ERROR_OUT("player error mail num zero:~p,~p", [playerState:getRoleID(), Attach]),
											0
									end,
								case getMailItem(MailID, ItemUID) of
									ok ->
										{updateList(false, Items, Attach#recMailAttachMent.mvalue2, ItemNumber), Coins};
									_ ->
										{false, playerTrade:getBagNumber(ItemID) > 0}
								end
						end,
					case Ret of
						false ->
			...
			F =
				fun(#recMail{mailID = MID, attachment = AttachMent, mailReadTime = ReadTime} = Mail, {false, {Number, Is, Cs}} = Acc) ->
					case AttachMent of
						[] ->
							Acc;
						_ ->
							...
							case ReadTime > 0 of
								true ->
									skip;
								false ->
									dbLog:sendSaveLogMailChange(playerState:getRoleID(), MID, ?MAIL_OPEN_AUTO),
									%% 第一次阅读，需要通知cs
									psMgr:sendMsg2PS(?PsNameMail, readMailNoAck, {Mail#recMail.senderRoleID, playerState:getRoleID(), MID})
							end,

							%% 提取附件
							case misc:foldlEx(FAttach, {false, {Is, Cs}}, AttachMent) of
								{false, {NewItems, NewCoins}} ->
									{false, {Number + 1, NewItems, NewCoins}};
								{true, {NewItems, NewCoins}} ->
									%% 结束
									{true, {Number, NewItems, NewCoins}};
								{true, {NewItems, NewCoins, Data2}} ->
									%% 结束
									{true, {Number, NewItems, NewCoins, Data2}}

							end
					end
				end,
			{Bool, Msg, PData} =
				case misc:foldlEx(F, {false, {0, [], []}}, List) of
					{Bl, {MailNumber, MailItems, MailCoins}} ->
						{Bl, #pk_GS2U_GetMailItemAll{number = MailNumber, items = MailItems, coins = MailCoins}, true};
					{Bl, {MailNumber, MailItems, MailCoins, PackageData}} ->
						{Bl, #pk_GS2U_GetMailItemAll{number = MailNumber, items = MailItems, coins = MailCoins}, PackageData}
				end,
			playerMsg:sendNetMsg(Msg),

			%% 提示
			if
				MailItems =:= [] andalso MailCoins =:= [] ->
					playerMsg:sendErrorCodeMsg(?ErrorCode_MailGetItemAllFailed);
				Bool andalso (not PData) ->
					%% 背包满导致的提取失败
