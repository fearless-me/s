...
playerRealEnterMap() ->
	...
	case getCfg:getCfgByKey(cfg_mapsetting, MapId) of
		#mapsettingCfg{type = ?MapTypeNormal} ->
			L = playerPropSync:getProp(?SerProp_AlreadyEnterNormalMap),
			case lists:member(MapId, L) of
				true ->
					skip;
				_ ->
					playerPropSync:setAny(?SerProp_AlreadyEnterNormalMap, [MapId | L])
			end;
		_ ->
			skip
	end,
...
telesport(PosX, PosY) ->
    ...
    case Can of
        true ->
            Code = playerState:getPlayerCode(),
            ?DEBUG_OUT("telesport roleID=~p, code=~p mapID=~p, tx=~p, ty=~p",
            ...
            ok;
        _ ->
            ?DEBUG_OUT("telesport failed roleID=~p", [playerState:getRoleID()]),
            skip
    end,
    ok.
...
sendSaveBuff() ->
    ...
    Fun =
        fun(#rec_buff{buffID = BuffID, endtime = EndTime}) ->
            case getCfg:getCfgPStack(cfg_buff, BuffID) of
                #buffCfg{buffstore = BuffStore, buffDuration = Duration} ->
                    case BuffStore of
                        ...
                        ?StoreUnTime -> %%剩余时间
                            ?DEBUG_OUT("save buff [~p], storeunTime end time [~p]", [BuffID, EndTime]),
                            case Duration of
                                [0, _AnyTime] ->
...
%%	end,
    ...
    ?DEBUG_OUT("[DebugForDaily] will playerDaily:sendToClient/0"),
