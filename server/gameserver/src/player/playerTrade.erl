...
%% 自定义物品类型列表
-define(ItemClassList, [?Item_Item, ?Item_Gem, ?Item_Rune]).
...
request(Msg) ->
	?DEBUG_OUT("request trade roleID=~p, Msg=~p", [playerState:getRoleID(), Msg]),
	IsCheckCD =
		case Msg of
...
referenceItem(#pk_U2GS_ReferenceItem{tradeClass = TradeClass, itemID = ItemID, requestNumber = Number}) ->
	...
	RItem =
		case ets:lookup(tradeDealRecord, ItemID) of
			[] ->
				#pk_ReferenceItem{itemID = ItemID, sellNumber = 0, gold = 0, diamond = 0};
			L ->
...
queryTrade(#pk_U2GS_QueryTrade{
	...
	case canQuery(TradeClass, ItemClass) of
		...
		true ->
			...
			%%?DEBUG_OUT("queryTradeDataEnter:~p", [L2]),
...
canPutTrade(TradeClass, ItemUID, ItemID, SellNumber, SellTime, Silver, Gold, DestRoleID) ->
	...
	if
		...
		TradeClass =:= ?TradeType_Gold ->
			%% 不允许金币交易
			{false, ?ErrorCode_TradeError};
		TradeClass =:= ?TradeType_Private andalso Gold =< 0 ->
			%% 指定交易时，也只能用钻石交易
			{false, ?ErrorCode_TradeError};
		TradeClass =:= ?TradeType_Diamond andalso Gold =< 0 ->
			%%元宝交易行时，元宝为0
			{false, ?ErrorCode_TradeError};
					...
					true ->
						...
						if
							...
							Gold > 0 ->
								...
								%% 需要提示扣除多少钱
								Tax = decTax(Gold),
								Attach = #recMailCoin{coinType = ?CoinTypeDiamond, coinNum = Gold - Tax},
...
sendResultToClient() ->
	...
	%%?DEBUG_OUT("sendResultToClient:~p", [Rec]),
	ok.
