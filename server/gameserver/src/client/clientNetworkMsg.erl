...
%% 平台登录分发
doTcpMsg(?UserState_UnCheckPass, ?CMD_U2LS_Login_Normal , #pk_U2LS_Login_Normal{versionPro = VPro} = Msg, #clientState{clientSocket = Socket} = State) ->
	...
	case globalSetup:getHasGs() of
		Val when erlang:is_number(Val) ->
			...
			case clientLogic:checkProtoVer([], VPro) of
				...
				_ ->
					?ERROR_OUT("No match ProtoVer:~ts,~ts", [Msg#pk_U2LS_Login_Normal.platformAccount, Msg#pk_U2LS_Login_Normal.platformName]),
