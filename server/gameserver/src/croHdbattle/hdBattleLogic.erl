...
startAc(false, EndTime) ->
	?LOG_OUT("gm corss hd battle ac will start !, cd time [~p]", [EndTime]),
	clear(),
	setPhaseOneCD([]),
	setPhaseThreeCD([]),
	setAcSwitch(true),
...
startAc(true, EndTime) ->
	...
	case lists:keyfind(Day, 1, L) of
		{Day, _} ->
			?LOG_OUT("corss hd battle ac will start !, cd time [~p]", [EndTime]),
			clear(),
			clearRank(Day),
			setPhaseOneCD([]),
			setAcSwitch(true),
			...
			skip;
		_ ->
			?LOG_OUT("corss hd battle ac today not open [~p]", [Day]),
			skip
	end.

-spec closeAc(Flag::boolean()) -> ok.
closeAc(false) ->
	?LOG_OUT("gm corss hd battle ac will close !"),
	setAcSwitch(false),
	setStatPlayerNum(0),
	%%清理阶段
	setPhaseOneCD([]),
...
closeAc(true) ->
	...
	case lists:keyfind(Day, 1, L) of
		{Day, _} ->
			?LOG_OUT("corss hd battle ac will close !"),
			setAcSwitch(false),
			%%清理阶段
			setPhaseOneCD([]),
			setPhaseThreeCD([]),
			...
			clear();
		_ ->
			?LOG_OUT("corss hd battle ac today not close [~p]", [Day]),
			skip
	end.
