...
activityChangeCallBack2(?ActivityPhase_GuildBattle_1) ->
	acGuildBattleState:setEndTime(0),
...
activityChangeCallBack2(?ActivityPhase_GuildBattle_2) ->
	%% 分组心跳
	erlang:send_after(1000, self(), guildBattleGroup),
	ok;
activityChangeCallBack2(?ActivityPhase_GuildBattle_3) ->
	...
	CT = case getCfg:getCfgByKey(cfg_activity, ?ActivityType_GuildBattle) of
			 #activityCfg{stage = TimeList} when erlang:length(TimeList) >= ?ActivityPhase_GuildBattle_3 ->
				 lists:nth(?ActivityPhase_GuildBattle_3, TimeList);
			 _ ->
				 0
		 end,
	acGuildBattleState:setEndTime(time2:getTimestampSec() + CT),
	...
	%% 全部设置为开始,
	Size = erlang:length(acGuildBattleState:getGuildPairs()),
	case Size > 0 of
		true ->
			#activityCfg{mapidlist = [MapID]} = getCfg:getCfgPStack(cfg_activity, ?ActivityType_GuildBattle),

			%% 创建活动地图
			core:sendMsgToMapMgr(MapID, createActivityMap, {MapID, Size, 0}),
			ok;
		_ ->
			skip
	end,
...
guildBattleHeart() ->
	case acGuildBattleState:isBattleIng() of
		true ->
			...
			LastTime = acGuildBattleState:getFreshOccupyTime(),
			case NowTime - LastTime >= 5 of
				true ->
					acGuildBattleState:setFreshOccupyTime(NowTime),

					L = acGuildBattleState:getGuildPairs(),
					F =
						fun(#battleGuildPair{} = Pair) ->
							broadcastBannerChange(Pair)
						end,
					lists:foreach(F, L),
					ok;
				_ ->
			...
			case lists:keyfind(GatherID, 1, L) of
				...
				_ ->
					...
					case NewPair2 /= Pair of
						true ->
							%% 广播旗帜拥有者
							broadcastBannerChange(NewPair2),
							acGuildBattleState:addGuildPairs(NewPair2);
						_ ->
							skip
					end
		...
		[#rec_guildBattle{waitEnterPlayerIDs = Waits, targetGuildID = TGuildID} = Battle] ->
			case lists:member(RoleID, Waits) of
				true ->
					...
					%% 广播旗帜拥有者
					Msg = make_pk_GS2U_OccupyOwnerChange(GuildID, RoleID),
...
make_pk_GS2U_OccupyOwnerChange(GuildID, RoleID) ->
	case ets:lookup(rec_guildBattle, GuildID) of
		[#rec_guildBattle{targetGuildID = TGuildID, allpoint = AllPoint}] ->
			%% 广播旗帜拥有者
			Pair = acGuildBattleState:getGuildPairsByGuildID(GuildID),
			{OccupyList, PlayerList} = getOccupyOwnerChangeMsg(GuildID, Pair),

			O1 = #pk_OccupyGuildInfo{
				guildID = GuildID,
				guildName = guildLogic:getGuildName(GuildID),
				occupyIDs = OccupyList,
				allPoint = AllPoint,
				denoter = guildLogic:getGuildDenoter(GuildID)
			},

			{PlayerList2, O3} =
				case ets:lookup(rec_guildBattle, TGuildID) of
					[#rec_guildBattle{allpoint = AllPoint2}] ->
						{OList, PList} = getOccupyOwnerChangeMsg(TGuildID, Pair),
						O2 = #pk_OccupyGuildInfo{
							guildID = TGuildID,
							guildName = guildLogic:getGuildName(TGuildID),
							occupyIDs = OList,
							allPoint = AllPoint2,
							denoter = guildLogic:getGuildDenoter(TGuildID)
						},
						{PlayerList ++ PList, O2};
					_ ->
						O2 = #pk_OccupyGuildInfo{
			...
			FList =
				fun(#pk_GuildBattlePlayerInfo{} = IInfo, {AccIndex, AccList}) ->
					{AccIndex + 1, [IInfo#pk_GuildBattlePlayerInfo{rank = AccIndex} | AccList]}
				end,
			{_, PlayerList222} = lists:foldl(FList, {1, []}, lists:reverse(lists:keysort(#pk_GuildBattlePlayerInfo.point, PlayerList2))),

			PlayerList3 = lists:sublist(lists:reverse(PlayerList222), 6),
			PlayerList4 = case lists:keyfind(RoleID, #pk_GuildBattlePlayerInfo.roleID, PlayerList3) of
							  #pk_GuildBattlePlayerInfo{} -> PlayerList3;
							  _ ->
								  case lists:keyfind(RoleID, #pk_GuildBattlePlayerInfo.roleID, PlayerList2) of
									  #pk_GuildBattlePlayerInfo{} = Info -> PlayerList3 ++ [Info];
									  _ -> PlayerList3
								  end
						  end,

			NowTime = time2:getTimestampSec(),
			EndTime = acGuildBattleState:getEndTime(),

			#pk_GS2U_OccupyOwnerChange{
				remainSec = erlang:max(0, EndTime - NowTime),
				owner = O1,
				target = O3,
				playerInfos = PlayerList4
			};
		_ ->
			OO = #pk_OccupyGuildInfo{
				guildID = 0,
				guildName = "",
				occupyIDs = [],
				allPoint = 0,
	...
	case ets:lookup(rec_guild_member, RoleID) of
		[#rec_guild_member{guildID = GID1}] ->
			NewPL1 = case lists:keyfind(RoleID, #enterGuildBattlePlayer.roleID, PL1) of
							 ...
							 %% 提示获得积分
							 playerMsg:sendErrorCodeMsg(Player1#enterGuildBattlePlayer.netPid, ?ErrorCode_GuildButtle_Point, [Point]),
		...
		[#rec_guild_member{guildID = GID2}] ->
			NewPL2 = case lists:keyfind(RoleID, #enterGuildBattlePlayer.roleID, PL2) of
							 ...
							 %% 提示获得积分
							 playerMsg:sendErrorCodeMsg(Player2#enterGuildBattlePlayer.netPid, ?ErrorCode_GuildButtle_Point, [Point]),
...
broadcastBannerChange(#battleGuildPair{mapPID = MapPID, guildID1 = GID1, guildID2 = GID2}) ->
	case misc:is_process_alive(MapPID) of
		true ->
			Msg1 = make_pk_GS2U_OccupyOwnerChange(GID1, 0),
			Msg2 = make_pk_GS2U_OccupyOwnerChange(GID2, 0),
			PlayerETS = acGuildBattleState:getMapPlayerEts(MapPID),
			F =
				fun(#recMapObject{netPid = NetPID, guild = GID}, _) ->
					case GID of
						GID1 -> gsSendMsg:sendNetMsg(NetPID, Msg1);
						_ -> gsSendMsg:sendNetMsg(NetPID, Msg2)
					end
				end,
			ets:foldl(F, 0, PlayerETS);
		_ ->
...
getOccupyOwnerChangeMsg(GuildID, #battleGuildPair{guildID1 = GID1, guildID2 = GID2} = Pair) ->
	[{G1,_P1}, {G2,_P2}, {G3,_P3}] = acGuildBattleState:getCollectIDs(),
	L1 = case Pair#battleGuildPair.collect1GuildID of
			 GuildID -> [G1];
			 _ -> []
		 end,

	L2 = case Pair#battleGuildPair.collect2GuildID of
			 GuildID -> [G2 | L1];
			 _ -> L1
		 end,

	L3 = case Pair#battleGuildPair.collect3GuildID of
			 GuildID -> [G3 | L2];
			 _ -> L2
		 end,

	PlayerInfo = case GuildID of
					 GID1 -> getPlayerBattleInfoList(Pair#battleGuildPair.players1);
					 _ -> getPlayerBattleInfoList(Pair#battleGuildPair.players2)
				 end,
	{L3, PlayerInfo}.
...
getGuildBattleInfo(#rec_guildBattle{mapPID = MapPID, guildID = GuildID, allpoint = Point}) ->
	...
	PlayerInfo = getPlayerBattleInfoList(PS),
	GuildName = guildLogic:getGuildName(GuildID),
	#pk_GuildBattleResultInfo{guildID = GuildID, guildName = GuildName, point = Point, playerInfos = PlayerInfo}.

-spec getPlayerBattleInfoList([#enterGuildBattlePlayer{},...]) -> [#pk_GuildBattlePlayerInfo{},...].
getPlayerBattleInfoList(PS) ->
	%% 排序
	PS_Sort = lists:keysort(#enterGuildBattlePlayer.point, PS),

	F =
		fun(#enterGuildBattlePlayer{roleID = RoleID, roleName = RoleName, point = P, guildID = G}, {Acc, Index}) ->
	...
	{PlayerInfo, _} = lists:foldl(F, {[], 1}, lists:reverse(PS_Sort)),
	lists:reverse(PlayerInfo).