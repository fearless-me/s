...
compile(Lang, V) ->
	...
	try
		...
		_ : Why ->
			io:format("**** ERROR ERROR ERROR ~p,~p ~n", [Why,erlang:get_stacktrace()])
	end,
...
compile2(SrcFile, Opts, LogFile) ->
	...
	d_log(LogFile, "compile:file(~ts, [return, error_summary | ~w])~n", [SrcFile, Opts]),
	d_log(LogFile, "[~ts] compiled ~s ~ts~n",
